...
class VendorRegistrationController(http.Controller):
    ...
    @http.route("/customer/apply", type="http", auth="public", website=True, sitemap=False)
    def customer_apply(self, **kwargs):
    ...
    @http.route("/customer/submit", type="http", auth="public", methods=["POST"], csrf=True, website=True)
    def customer_submit(self, **post):
        ...
        vals = {
            ...
            "city": post.get("city"),
            "state_id": int(post.get("state") or 0),
            "zip": post.get("zip"),
            "country_id": int(post.get("country") or 0),
            ...
            "payment_condition": post.get("payment_condition"),
            # "price_list_discount": post.get("price_list_discount"),
        ...
        vals.update({
            ...
            "type": "opportunity",
            "tag_ids": [(4, tag.id)],
            "is_customer_reg": True,
        })
        lead = request.env["crm.lead"].sudo().create(vals)

        return request.redirect(f'/customer/thank-you?lead={lead.id}')

    @http.route('/customer/thank-you', type='http', auth='public', website=True, sitemap=False)
    def customer_thank_you(self, **kw):
